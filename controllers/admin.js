...
module.exports = {
    ...
    postChangePassword:function(req,res){
           ...
           User.findByPk(user).then((result)=>{
               console.log("change password outside",result.dataValues.password,result.dataValues.salt)
               const finalresult = Utils.comparePassword(oldPassword,result.dataValues.password,result.dataValues.salt)
               ...
               User.update({password:encrypt.password,salt:encrypt.salt},{where:{id:result.dataValues.id}}).then((user)=>{
                   if(!result){
